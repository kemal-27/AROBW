<?php
    if(isset($_POST["submit_all"]))
    {
        if($_GET['sub'] == "data_pegawai")
        {
            ...
            if($nama_admin == '')
            {
                $error_nama_admin = "Nama Pegawai diperlukan";
            }
            if($address == '')
            {
                $error_address = "Address Pegawai diperlukan";
            }
            if($usernamePegawai == '')
            {
                $error_username = "Username Pegawai diperlukan";
            }
            if($password == '')
            {
                $error_password = "Password Pegawai diperlukan";
            }
            if($email == '')
            {
                $error_email = "Email Pegawai diperlukan";
            }
           
            if($error_nama_admin || $error_address || $error_email || $error_username || $error_password )
            {
            }
            else
            {
                $query = " SELECT MAX(id_admin) as MAX FROM admin";
                $temp_result = mysqli_query($con, $query);
                if(!$temp_result)
                {
                    echo("Could not query the database: <br />". mysqli_error($con));
                    echo "<p><a href=\"dashboard.php?halaman=add_database&id=$username&sub=data_pegawai\">Kembali</a>";
                    die();
                }
                $temp_row = mysqli_fetch_array($temp_result);
                $increment = 0;
                $increment = $temp_row['MAX'] + 1;
                $query = " ALTER TABLE admin AUTO_INCREMENT=".$temp_row['MAX'];
                $temp_result = mysqli_query($con, $query);
                if(!$temp_result)
                {
                    echo("Could not query the database: <br />". mysqli_error($con));
                ...
                $query = " ALTER TABLE profile AUTO_INCREMENT=".$temp_row['MAX'];
                $temp_result = mysqli_query($con, $query);
                if(!$temp_result)
                {
                    echo("Could not query the database: <br />". mysqli_error($con));
                ...
                $query = " INSERT INTO admin (id_admin, nama_admin, address, username, password, email) VALUES (NULL, '$nama_admin', '$address', '$usernamePegawai', '$password', '$email')";
                $query2 = "INSERT INTO profile (idprofile, id_admin, address, city, country,postalcode,about, gambar) VALUES ($increment, $increment, '', '', '', 0, '', '$gambar_profil')";
        ...
        else if($_GET['sub'] == "data_siswa")
        {
            ...
            {
                $query = " INSERT INTO siswa (nisn, nama_siswa, tgl_lahir, jenis_kelamin, alamat, agama, anak_ke, status, nama_ayah, nama_ibu, pekerjaan_ayah, pekerjaan_ibu, no_telp, tgl_diterima, sekolah_asal, kd_kelas, username, password, gambar) VALUES ('$nisn', '$nama_siswa', '$tgl_lahir', '$jenis_kelamin', '$alamat', '$agama', '$anak_ke', '$status', '$nama_ayah', '$nama_ibu', '$pekerjaan_ayah', '$pekerjaan_ibu', '$no_telp', '$tgl_diterima', '$sekolah_asal', '$kd_kelas', '$usernameSiswa', '$password','$gambar_profil')";
                $result = mysqli_query($con, $query);
                if(!$result)
                {
                    echo("Could not query the database: <br />". mysqli_error($con));
...
<?php
    if($_GET['sub'] == "data_pegawai")
    {
        ...
        <form method="POST" enctype="multipart/form-data" autocomplete="on" action="'; echo htmlspecialchars("dashboard.php?halaman=add_database&id=$username&sub=data_pegawai"); echo '">
            <table id="dataTableAdd" width="auto" cellspacing="0">
                <tr>
                    <td valign="top">Nama</td>   
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="nama_admin" size="30" maxlength="100" placeholder="nama admin (max 100 characters)" autofocus>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <textarea name="address" size="30" rows="3" maxlength="200" placeholder="address  (max 200 characters)" autofocus></textarea>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="username" size="30" maxlength="30" placeholder="username (max 30 characters)" autofocus>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="password" size="30" maxlength="30" placeholder="password (max 30 characters)" autofocus>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="email" size="30" maxlength="50" placeholder="email (max 50 characters)" autofocus>
                    </td>
                    <td valign="top">
        ...
        <form method="POST" enctype="multipart/form-data" autocomplete="on" action="'; echo htmlspecialchars("dashboard.php?halaman=add_database&id=$username&sub=data_siswa"); echo '">
            <table id="dataTableAdd" width="auto" cellspacing="0">
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="nisn" size="30" maxlength="100" placeholder="nisn (max 13 characters)" autofocus>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="tgl_lahir" size="30" maxlength="30" placeholder="yyyy-mm-dd" autofocus>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td valign="top">:</td>   
                    <td valign="top">
                        <input type="text" name="password" size="30" maxlength="50" placeholder="password (max 50 characters)" autofocus></textarea>
                    </td>
                    <td valign="top">
                        <span style="color:red">*'; echo (isset($error_password)) ? $error_password : ""; echo '</span>
                    </td>  
